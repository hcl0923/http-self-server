...
//        jdk1.8的新的try...catch释放的方法
    ...
    private static Map<String,String> initXml(){
        ...
        //     dom, sax
        //   ***  dom4j   jdom
        DocumentBuilderFactory factory=DocumentBuilderFactory.newInstance();//通过DocumentBulderFactory创建XML解释器
        try{
            DocumentBuilder bulider=factory.newDocumentBuilder();//通过解释器创建一个可以加载并生成XML的DocumentBuilder
            //TODO: 发布后是否有路径问题
            Document doc=bulider.parse("conf/server.xml");//通过DocumentBuilder加载并生成一颗XML数，Document对象的实例
            //javascript中的dom的解释方式
            NodeList nl=doc.getElementsByTagName("Server");//通过Document可以遍历这棵树，并读取相应结点中的内容
